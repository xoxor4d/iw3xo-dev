...
namespace Game
{
	...
	{
		// vert + pixel
		vec4_t _WaveAmplitude	= { 0.72f,	0.40f,	0.46f,	1.93f };	// filterTap0
		vec4_t _WavesIntensity	= { 0.28f,	1.95f,	1.00f,	1.90f };	// filterTap1
		vec4_t _WavesNoise		= { 0.05f,	0.45f,	1.18f,	0.28f };	// filterTap2

		// vert ---------------------------------------------------------------------
		vec2_t _WindDirection		= { -0.85f,	-3.1f };				// filterTap3
		...
		// pixel --------------------------------------------------------------------
		vec3_t _AmbientColor	= { 1.0f, 1.0f, 1.0f };					// filterTap3
		float _AmbientDensity	= -0.0075f;
		
		vec3_t _ShoreColor		= { 1.0f, 1.0f, 1.0f };					// filterTap4
		float _DiffuseDensity	= -0.2649f;

		vec3_t _SurfaceColor	= { 0.871f, 0.953f, 0.804f };			// filterTap5
		float _NormalIntensity	= 0.9549f;

		vec3_t _DepthColor		= { 0.914f, 0.988f, 1.047f };			// filterTap6
		float _ShoreFade		= 0.0073f;

		vec3_t _RefractionValues = { -0.2637f, 0.4148f, 1.0f };			// filterTap7
		float _RefractionScale	 = 0.005f;

		vec3_t _HorizontalExtinction = { 100.0f, 250.0f, 500.0f };		// colorMatrixR
		...
		vec3_t _SpecularValues		= { 17.75f, 43.00f, 5.5f };			// colorMatrixB
		float _Shininess			= 0.68f;

		vec3_t _FoamTiling			= { 0.075f, 0.25f, -0.7f };			// dofEquationViewModelAndFarBlur
		float _FoamSpeed			= 150.0f;

		vec3_t _FoamRanges			= { 1.8f, 2.2f, 2.46f };			// dofEquationScene
		float _FoamIntensity		= 14.00f;

		vec4_t _FoamNoise			= { 1.0f, 1.00f, 1.00f, 1.00f };	// dofLerpScale
