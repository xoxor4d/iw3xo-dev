...
namespace components
{
	...
	game::debug_lines_delay_sync_s sync_debug_lines = game::debug_lines_delay_sync_s();
	game::debug_line_s line_trace_origin = game::debug_line_s();
	game::debug_line_s line_trace_velocity = game::debug_line_s();
	...
	void draw_world_axis()
	{
		...
		if (dvars::pm_debug_drawAxis->current.integer == 2)
		{
			...
			{
				game::dvar_set_value_dirty(dvars::pm_debug_drawAxis, 0);
	...
	void copy_local_playerstate_vars(game::pmove_t* pm)
	{
		...
		game::glob::lpmove_velocity = glm::vec3(pm->ps->velocity[0], pm->ps->velocity[1], pm->ps->velocity[2]);

		// Grab player origin and angles
		game::glob::lpmove_angles = glm::vec3(pm->ps->viewangles[0], pm->ps->viewangles[1], pm->ps->viewangles[2]);
		game::glob::lpmove_origin = glm::vec3(pm->ps->origin[0], pm->ps->origin[1], pm->ps->origin[2]);

		// Calculate camera origin
		game::glob::lpmove_camera_origin = game::glob::lpmove_origin + glm::vec3(0.0f, 0.0f, pm->ps->viewHeightCurrent);

		// not really needed and prob. inacurate because this function gets called 6 times
		// ----- Grab servertime / frametime stuff ------
		game::glob::lpmove_server_time_old = game::glob::lpmove_server_time;
		game::glob::lpmove_server_time = pm->cmd.serverTime;

		// -> calculate frametime
		game::glob::lpmove_server_frame_time = game::glob::lpmove_server_time - game::glob::lpmove_server_time_old;
	...
	void pmove_client(game::pmove_t *pm)
	{
		...
		const bool radiant_camera_active = game::glob::cgs_addons.radiant_livelink_connected && (dvars::radiant_syncCamera->current.integer == 2 || dvars::radiant_syncCamera->current.integer == 3);
		...
		if (components::active.radiant_livelink && radiant_camera_active)
		{
			...
			if (!game::glob::cgs_addons.radiant_camera_client_time_old)
			{
				game::glob::cgs_addons.radiant_camera_client_time_old = command_time;
			}
			else
			{
				current_diff = (command_time - game::glob::cgs_addons.radiant_camera_client_time_old);

				// add the previous frametime to our counter
				game::glob::cgs_addons.radiant_camera_client_msec_passed += current_diff;
			...
			if (pm->ps->commandTime != server_time)
			{
				while (true)
				{
					...
					if (components::active.radiant_livelink && radiant_camera_active)
					{
						...
						if (!utils::vector::compare_int3(pm->cmd.angles, pm->oldcmd.angles) || !utils::vector::compare3(temp_camera_origin, game::glob::cgs_addons.radiant_camera_origin_old))
						{
							memcpy(&game::glob::cgs_addons.radiant_camera_origin_old, temp_camera_origin, sizeof(int[3]));
							...
							if (current_diff >= refresh_rate_ms || game::glob::cgs_addons.radiant_camera_client_msec_passed <= refresh_rate_ms && !game::glob::cgs_addons.radiant_camera_capped_packets)
							{
								game::glob::cgs_addons.radiant_camera_capped_packets = true;
							...
							else if (game::glob::cgs_addons.radiant_camera_client_msec_passed >= refresh_rate_ms)
							{
								game::glob::cgs_addons.radiant_camera_client_msec_passed = 0;
								game::glob::cgs_addons.radiant_camera_capped_packets = false;
							}
						}
					}
