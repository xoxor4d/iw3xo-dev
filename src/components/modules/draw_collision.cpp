...
// view-frustum planes
const game::vec4_t frustum_side_planes_[5] = 
{
...
namespace components
{
	...
	void build_frustum_planes(game::GfxViewParms *view_parms, game::cplane_s *frustum_planes)
	{
		...
		frustum_clip_planes(&view_parms->viewProjectionMatrix, frustum_side_planes_, 5, dpvs_frustum_planes);
...
#define LODWORD(x)  (*((DWORD*)&(x)))  // low dword
	...
	int add_simple_brush_point(const game::cbrush_t* brush, const game::axialPlane_t* axial_planes, const __int16* side_indices, const float* xyz, int pt_count, game::ShowCollisionBrushPt* brush_pts)
	{
		if (!brush)
		{
			game::Com_Error(0, utils::va("add_simple_brush_point L#%d :: brush \n", __LINE__));
			return 0;
		}

		if (!brush_pts)
		{
			game::Com_Error(0, utils::va("add_simple_brush_point L#%d :: brushPts \n", __LINE__));
			return 0;
		}
	...
	bool build_brush_winding_for_side(game::winding_t* winding, const float* plane_normal, const int side_index, game::ShowCollisionBrushPt* pts, int pt_count)
	{
		...
		const glm::vec3 camera_direction_to_plane = xyz_list[0] - game::glob::lpmove_camera_origin;
	...
	void init_once_per_map()
	{
		...
		// only once per map
		if(!game::glob::debug_collision_initialized || utils::q_stricmp(map_clipmap_name_, game::cm->name))
		{
			...
			game::dvar_set_value_dirty(dvars::r_drawCollision_brushAmount, 1);
#endif
			...
			game::dvar_set_value_dirty(dvars::r_drawCollision_brushDist, 800.0f);
			...
			game::glob::r_drawCollision_materialList_string.clear();
			...
			// set globals
			game::glob::debug_collision_initialized = true;
			...
			for (auto num = 0u; num < single_material_count; num++) 
			{
				...
				{
					game::glob::r_drawCollision_materialList_string += std::to_string(num) + ": " + map_material_list_single_[num] + "\n";
				}
			}
			...
			game::dvar_set_value_dirty(dvars::r_drawCollision_material, 0);
			...
			{
				dvars::r_drawCollision_material->description = game::glob::r_drawCollision_materialList_string.c_str();
			}
	...
	void sort_brush_list_by_distance(bool far_to_near = true, bool use_current_brushes_drawn_list = false, bool always_update = false)
	{
		if (!use_current_brushes_drawn_list)
		{
			...
			if (last_sorting_origin_[0] != game::glob::lpmove_origin[0] || last_sorting_origin_[1] != game::glob::lpmove_origin[1] || always_update)
			{
				...
				if (far_to_near)
				{
					std::ranges::sort(map_brush_list_.begin(), map_brush_list_.end(), [](game::cbrush_t *brush1, game::cbrush_t *brush2)
					{
						...
						return glm::distance(b1_mid, game::glob::lpmove_origin) > glm::distance(b2_mid, game::glob::lpmove_origin);
					});
				}
				...
				last_sorting_origin_ = game::glob::lpmove_origin;
			}
		}
	...
	void patch_tri_from_incides(game::map_patchTris_t *ptris, const unsigned short *incides)
	{
		...
		for (auto gfx_vert = 0u; gfx_vert < game::gfx_world->vertexCount; gfx_vert++)
		{
			// try to match our first clipmap vertex to a gfxworld vertex
			if (!found_first && utils::vector::compare3(game::cm->verts[incides[0]], game::gfx_world->vd.vertices[gfx_vert].xyz))
			{
				// found the first corrosponding gfx vertex :: copy gfx vertex data to our temp vertex
				memcpy(&ptris->coords[0], &game::gfx_world->vd.vertices[gfx_vert], sizeof(game::GfxWorldVertex));
			...
			if (!found_second && utils::vector::compare3(game::cm->verts[incides[1]], game::gfx_world->vd.vertices[gfx_vert].xyz))
			{
				// found the second corrosponding gfx vertex :: copy gfx vertex data to our temp vertex
				memcpy(&ptris->coords[1], &game::gfx_world->vd.vertices[gfx_vert], sizeof(game::GfxWorldVertex));
			...
			if (!found_third && utils::vector::compare3(game::cm->verts[incides[2]], game::gfx_world->vd.vertices[gfx_vert].xyz))
			{
				// found the third corrosponding gfx vertex :: copy gfx vertex data to our temp vertex
				memcpy(&ptris->coords[2], &game::gfx_world->vd.vertices[gfx_vert], sizeof(game::GfxWorldVertex));
			...
			if (gfx_vert + 1 > game::gfx_world->vertexCount && !second_iteration)
			{
	...
	void brush_filters(int &filter_brush_amount, bool &filter_brush_index, bool &filter_brush_sorting, bool &filter_brush_selection)
	{
		...
		if (dvars::mapexport_selectionMode && dvars::mapexport_selectionMode->current.integer != 0)
		{
			...
			{
				game::dvar_set_value_dirty(dvars::r_drawCollision_brushAmount, total_brush_amount);
			}

			if (dvars::r_drawCollision_brushSorting && dvars::r_drawCollision_brushSorting->current.integer != 0)
			{
				game::dvar_set_value_dirty(dvars::r_drawCollision_brushSorting, 0);
			}
	...
	void handle_debug_collision_commands()
	{
		...
		if (dvars::r_drawCollision_materialList && dvars::r_drawCollision_materialList->current.enabled)
		{
			...
			game::dvar_set_value_dirty(dvars::r_drawCollision_materialList, false);
			game::Com_PrintMessage(0, utils::va("%s\n", game::glob::r_drawCollision_materialList_string.c_str()), 0);
		}
	}
		...
		if (is_active)
		{
			...
			if (!mapexport_selectionbox_.is_box_valid)
			{
				...
				if (mapexport_selectionbox_.num_points < 2)
				{
					...
					{
						mapexport_selectionbox_.points[mapexport_selectionbox_.num_points] = game::glob::lpmove_origin;
					...
					{
						mapexport_selectionbox_.points[1] = game::glob::lpmove_origin;
	...
	void draw_brush_collision(game::GfxViewParms *viewParms, game::cplane_s *frustumPlanes, int frustumPlaneCount)
	{
		...
		// reset hud element
		game::glob::debug_collision_rendered_planes_counter = 0;
		...
		if (brush_index_visible && !map_brush_list_for_index_filtering_.empty())
		{
			...
			{
				game::dvar_set_value_dirty(dvars::r_drawCollision_brushIndexVisible, false);
		...
		if (mapexport_in_progress_)
		{
			...
			if (dvars::mapexport_writeEntities->current.enabled)
			{
				...
				for (auto probe = 1u; probe < game::gfx_world->reflectionProbeCount; probe++)
				{
					if (filter_brush_selection && 
						!utils::polylib::is_point_within_bounds(glm::vec3(game::gfx_world->reflectionProbes[probe].origin[0],
																	 game::gfx_world->reflectionProbes[probe].origin[1],
																	 game::gfx_world->reflectionProbes[probe].origin[2]),
					...
					mapexport_mapfile_ << utils::va("\"origin\" \"%.1f %.1f %.1f\"", game::gfx_world->reflectionProbes[probe].origin[0], game::gfx_world->reflectionProbes[probe].origin[1], game::gfx_world->reflectionProbes[probe].origin[2]) << std::endl;
			...
			if (dvars::mapexport_writeModels->current.enabled)
			{
				...
				for (auto smodel = 0u; smodel < game::gfx_world->dpvs.smodelCount; smodel++)
				{
					// only export static models within the selection box
					if (filter_brush_selection &&
						!utils::polylib::is_point_within_bounds(glm::vec3(game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[0],
																	 game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[1],
																	 game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[2]),
																	 mapexport_selectionbox_.mins, mapexport_selectionbox_.maxs, 0.25f)) 
					{
					...
					// X
					matrix[0][0] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[0][0];
					matrix[0][1] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[0][1];
					matrix[0][2] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[0][2];

					// Y
					matrix[1][0] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[1][0];
					matrix[1][1] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[1][1];
					matrix[1][2] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[1][2];

					// Z
					matrix[2][0] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[2][0];
					matrix[2][1] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[2][1];
					matrix[2][2] = game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.axis[2][2];
					...
					mapexport_mapfile_ << utils::va("\"modelscale\" \"%.1f\"", game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.scale) << std::endl;
					mapexport_mapfile_ << utils::va("\"origin\" \"%.1f %.1f %.1f\"", game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[0], game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[1], game::gfx_world->dpvs.smodelDrawInsts[smodel].placement.origin[2]) << std::endl;
					mapexport_mapfile_ << utils::va("\"angles\" \"%.1f %.1f %.1f\"", angles[1], angles[2], angles[0]) << std::endl;
					mapexport_mapfile_ << utils::va("\"model\" \"%s\"", game::gfx_world->dpvs.smodelDrawInsts[smodel].model->name) << std::endl;
		...
		if (game::glob::debug_collision_rendered_planes_amount != game::glob::debug_collision_rendered_planes_counter) 
		{
			game::glob::debug_collision_rendered_planes_amount = game::glob::debug_collision_rendered_planes_counter;
		}

		if (game::glob::debug_collision_rendered_brush_amount != last_drawn_brush_amount) 
		{
			game::glob::debug_collision_rendered_brush_amount = last_drawn_brush_amount;
		}
	}
	...
	void draw_collision::debug_collision_frame(game::GfxViewParms *viewParms)
	{
		...
		if (dvars::r_drawCollision && dvars::r_drawCollision->current.integer > 0)
		{
			...
			if (game::Dvar_FindVar("r_fullbright")->current.enabled) 
			{
				...
				game::dvar_set_value_dirty(dvars::r_drawCollision, 0);
