...
namespace components
{
	...
	// skip com_error func in 'AimTarget_GetTagPos' and just return the result 'false'
	__declspec(naked) void AimTarget_GetTagPos_stub()
	{
		const static uint32_t retn_addr = 0x40251C;
		__asm
		{
			jmp		retn_addr;
		}
	...
	patches::patches()
	{
		...
		// disable Com_Error calls in 'AimTarget_GetTagPos' 
		utils::hook::nop(0x402507, 6); utils::hook(0x402507, AimTarget_GetTagPos_stub, HOOK_JUMP).install()->quick();
